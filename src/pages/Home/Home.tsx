...
// Selección combinada y equilibrada entre perfumes Árabes e Internacionales para Hombre, Mujer y Unisex
const NEW_ARRIVALS = [
  CATALOG_PRODUCTS.find(p => p.id === 205) || CATALOG_PRODUCTS[0], // Sauvage - Christian Dior (Hombre - Internacional)
  CATALOG_PRODUCTS.find(p => p.id === 320) || CATALOG_PRODUCTS[1], // 9 AM Rosa - Afnan (Mujer - Árabe)
  CATALOG_PRODUCTS.find(p => p.id === 326) || CATALOG_PRODUCTS[2], // Amber Rouge - Orientica (Unisex - Árabe)
  CATALOG_PRODUCTS.find(p => p.id === 100) || CATALOG_PRODUCTS[3], // Cloud Pink - Ariana Grande (Mujer - Internacional)
];

const BEST_SELLERS = [
  CATALOG_PRODUCTS.find(p => p.id === 336) || CATALOG_PRODUCTS[4], // #1 Club de Nuit Intense - Armaf (Hombre - Árabe)
  CATALOG_PRODUCTS.find(p => p.id === 137) || CATALOG_PRODUCTS[5], // #2 Good Girl - Carolina Herrera (Mujer - Internacional)
  CATALOG_PRODUCTS.find(p => p.id === 352) || CATALOG_PRODUCTS[6], // #3 Khamrah - Lattafa (Unisex - Árabe)
];
...
const COLLECTIONS = [
  { 
    ...
    name: 'Amaderados', 
    icon: Flame, 
    description: 'Notas cálidas de sándalo, cedro y resinas místicas.', 
    image: 'https://images.unsplash.com/photo-1546484475-7f7bd55792da?auto=format&fit=crop&q=80&w=1000' 
  },
  { 
    id: 2, 
    name: 'Florales', 
    icon: Sparkles, 
    description: 'Pétalos de rosa, jazmín silvestre y elegancia atemporal.', 
    image: 'https://images.unsplash.com/photo-1563241527-3004b7be0ffd?auto=format&fit=crop&q=80&w=1000' 
  },
  { 
    id: 3, 
    name: 'Cítricos', 
    icon: Wind, 
    description: 'Bergamota fresca, lima jugosa y energía chispeante.', 
  ...
  { 
    ...
    name: 'Acuáticos', 
    icon: Droplets, 
    description: 'Brisa marina, sal de océano y pureza cristalina.', 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {COLLECTIONS.map((col) => {
              ...
              return (
                <Link 
                  key={col.id} 
                  to={`/catalog?family=${encodeURIComponent(col.name)}`}
